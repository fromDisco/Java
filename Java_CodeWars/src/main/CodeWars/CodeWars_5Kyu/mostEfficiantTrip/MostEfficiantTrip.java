...
/**
 *
 ohn and Mary want to travel between a few towns A, B, C ...
 ...
 */
public class MostEfficiantTrip {
    public static Integer chooseBestSum(int maxMiles, int numOfTowns, List<Integer> distances) {
        int len = distances.size();
        List<Integer> possibleMilages = new ArrayList<>();
        for (int i = 0; i < len - 2; i++) {
            int counter = 1;
            while (i + counter <= len - (numOfTowns - 1)) {
                int collector = distances.subList(i, i + numOfTowns).stream().reduce(0, Integer::sum);
                collector = distances.get(i) + distances.get(i + counter) + distances.get(i + counter + 1);
                if (collector <= maxMiles) {
                    possibleMilages.add(collector);
                }
                counter++;
            }
        }
        return Collections.max(possibleMilages);
    }
}
